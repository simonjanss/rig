...
-- update copies the row as it was before writing the change, so the history is
-- rows in the same table rather than a diff nobody can read.
--
-- `rig migration new <name> --table t --snapshot` scaffolds this on a new
-- table. Adding it to one that already exists is the same three columns and the
-- same constraint, spelled as an ALTER.
