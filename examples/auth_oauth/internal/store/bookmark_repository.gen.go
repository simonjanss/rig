...
// BookmarkRepository reads and writes bookmark rows.
//
...
type BookmarkRepository interface {
	...
	Get(ctx context.Context, id uuid.UUID, opts ...readopt.Option) (*model.Bookmark, error)
	...
	List(ctx context.Context, f model.BookmarkFilter, page model.BookmarkPage, opts ...readopt.Option) ([]*model.Bookmark, int64, error)
	...
	Create(ctx context.Context, in dbhook.Create[model.BookmarkCreateInput, model.Bookmark]) (*model.Bookmark, error)

	// Update changes the fields the input mentions and leaves the rest alone.
	Update(ctx context.Context, id uuid.UUID, in dbhook.Update[model.BookmarkUpdateInput, model.Bookmark]) (*model.Bookmark, error)

	// Delete removes a row.
	Delete(ctx context.Context, in dbhook.Delete[model.BookmarkDeleteInput, model.Bookmark]) error
}

// bookmarkGroup turns a filter into the condition tree the runtime renders.
//
...
func bookmarkGroup(f model.BookmarkFilter, sc filterScope) (query.Group, error) {
	...
	if p := f.Equals; p != nil {
		...
		if p.URL != nil {
			g.Add(sc.at(query.Eq("url", *p.URL)))
		}
		if p.CreatedAt != nil {
	...
	if p := f.NotEquals; p != nil {
		...
		if p.URL != nil {
			g.Add(sc.at(query.Ne("url", *p.URL)))
		}
		if p.CreatedAt != nil {
	...
	if p := f.Contains; p != nil {
		...
		if len(p.URL) > 0 {
			g.Add(sc.at(query.In("url", p.URL)))
		}
	...
	if p := f.NotContains; p != nil {
		...
		if len(p.URL) > 0 {
			g.Add(sc.at(query.NotIn("url", p.URL)))
		}
		if len(p.CreatedAt) > 0 {
	...
	if p := f.Like; p != nil {
		...
		if p.URL != nil {
			g.Add(sc.at(query.Like("url", *p.URL)))
		}
	}
	if p := f.NotLike; p != nil {
		...
		if p.URL != nil {
			g.Add(sc.at(query.NotLike("url", *p.URL)))
		}
	}
	if p := f.Null; p != nil {
		if p.CreatedByAccountID != nil {
...
// bookmarkSortable reports whether a column can be ordered by.
func bookmarkSortable(column string) bool {
	switch column {
	case "id", "tenant_id", "title", "url", "created_at", "created_by_account_id", "updated_at", "updated_by_account_id":
		return true
	}
	return false
}

// bookmarkOrder converts the model's ordering terms into the runtime's.
func bookmarkOrder(terms []model.BookmarkOrder, sc filterScope) ([]query.Order, []query.Join, error) {
	if len(terms) == 0 {
		return nil, nil, nil
	}
	...
	for _, t := range terms {
		...
		if !bookmarkSortable(t.Column) {
			return nil, nil, rigerr.Invalid("a Bookmark cannot be ordered by %q", t.Column)
		}
...
var _ BookmarkRepository = (*bookmarkRepo)(nil)

const bookmarkRepoSelect = "bookmark.id, bookmark.tenant_id, bookmark.title, bookmark.url, bookmark.created_at, bookmark.created_by_account_id, bookmark.updated_at, bookmark.updated_by_account_id"

// scanBookmark reads one row in the order bookmarkRepoSelect lists.
func scanBookmark(row pgx.Row) (*model.Bookmark, error) {
	var m model.Bookmark
	if err := row.Scan(&m.ID, &m.TenantID, &m.Title, &m.URL, &m.CreatedAt, &m.CreatedByAccountID, &m.UpdatedAt, &m.UpdatedByAccountID); err != nil {
		return nil, err
	}
...
// Get implements BookmarkRepository.
func (r *bookmarkRepo) Get(ctx context.Context, id uuid.UUID, opts ...readopt.Option) (*model.Bookmark, error) {
	...
	sql := fmt.Sprintf("SELECT %s FROM bookmark WHERE %s", bookmarkRepoSelect, where)
	m, err := scanBookmark(r.db.connFor(ctx).QueryRow(ctx, sql, args...))
	if dbx.IsNoRows(err) {
		return nil, rigerr.NotFound("no Bookmark with id %s", id)
	}
	if err != nil {
		return nil, rigerr.Internal(err, "read bookmark")
	}
	return m, nil
}

// List implements BookmarkRepository.
func (r *bookmarkRepo) List(ctx context.Context, f model.BookmarkFilter, page model.BookmarkPage, opts ...readopt.Option) ([]*model.Bookmark, int64, error) {
...
func (r *bookmarkRepo) list(ctx context.Context, f model.BookmarkFilter, page model.BookmarkPage, opts []readopt.Option) ([]*model.Bookmark, int64, error) {
	...
	sc := newFilterScope(claims, "bookmark")
	...
	countSQL := fmt.Sprintf("SELECT count(*) FROM bookmark%s", countWhere)
	var total int64
	if err := r.db.connFor(ctx).QueryRow(ctx, countSQL, countArgs.Values()...).Scan(&total); err != nil {
		return nil, 0, rigerr.Internal(err, "count bookmark")
	}

	order, joins, err := bookmarkOrder(page.OrderBy, sc)
	if err != nil {
		return nil, 0, err
	}
	if len(order) == 0 {
		order = BookmarkDefaultOrder
	}
	...
	listSQL := fmt.Sprintf("SELECT %s FROM bookmark%s%s%s%s", bookmarkRepoSelect, joinSQL, where, query.OrderSQL(order), window.SQL(args))
	rows, err := r.db.connFor(ctx).Query(ctx, listSQL, args.Values()...)
	if err != nil {
		return nil, 0, rigerr.Internal(err, "list bookmark")
	}
	defer rows.Close()

	out := make([]*model.Bookmark, 0, window.Limit)
	for rows.Next() {
		m, err := scanBookmark(rows)
		if err != nil {
			return nil, 0, rigerr.Internal(err, "read bookmark")
		}
		out = append(out, m)
	}
...
// BookmarkDefaultOrder is the ordering used when a query asks for none.
//
// It always ends with the primary key, so the order is total and a page
// boundary cannot repeat or skip a row.
var BookmarkDefaultOrder = []query.Order{{Table: "bookmark", Column: "created_at", Desc: true}, {Table: "bookmark", Column: "id", Desc: false}}

// Create implements BookmarkRepository.
func (r *bookmarkRepo) Create(ctx context.Context, in dbhook.Create[model.BookmarkCreateInput, model.Bookmark]) (*model.Bookmark, error) {
	...
	err = dbx.InTxIf(ctx, r.db.pool, r.db.connFor(ctx), needsTx, func(ctx context.Context, tx dbx.Conn) error {
		...
		columns := []string{"id", "tenant_id", "created_at", "created_by_account_id", "title", "url"}
		values := []any{id, claims.TenantID, now, claims.Actor(), in.Input.Title, in.Input.URL}

		sql := fmt.Sprintf("INSERT INTO bookmark (%s) VALUES (%s) RETURNING %s", joinColumns(columns), placeholders(len(values)), bookmarkRepoSelect)

		created, err := scanBookmark(tx.QueryRow(ctx, sql, values...))
		if err != nil {
			return writeError(err, "bookmark")
		}
		m = created
...
// Update implements BookmarkRepository.
func (r *bookmarkRepo) Update(ctx context.Context, id uuid.UUID, in dbhook.Update[model.BookmarkUpdateInput, model.Bookmark]) (*model.Bookmark, error) {
	in.Input.Normalize()
	...
	var updated, prev *model.Bookmark
	err = dbx.InTx(ctx, r.db.pool, func(ctx context.Context, tx dbx.Conn) error {
		...
		sql := fmt.Sprintf("UPDATE bookmark SET %s WHERE id = $%d RETURNING %s", assignments(columns), len(values), bookmarkRepoSelect)

		updated, err = scanBookmark(tx.QueryRow(ctx, sql, values...))
		if err != nil {
			return writeError(err, "bookmark")
		}
...
// Delete implements BookmarkRepository.
func (r *bookmarkRepo) Delete(ctx context.Context, in dbhook.Delete[model.BookmarkDeleteInput, model.Bookmark]) error {
	claims, err := tenancy.FromContext(ctx)
	if err != nil {
		return err
	}
	...
	err = dbx.InTx(ctx, r.db.pool, func(ctx context.Context, tx dbx.Conn) error {
		...
		if _, err := tx.Exec(ctx, "DELETE FROM bookmark WHERE id = $1", in.Input.ID); err != nil {
			return writeError(err, "bookmark")
		}
		if in.Hooks.After != nil {
