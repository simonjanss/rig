...
// rules is Fixture's business logic.
//
...
var _ api.FixtureRules = (*rules)(nil)
...
// promoted method:
//
// type Service struct{ api.DefaultFixtureService } func (s *Service) Get(ctx
// context.Context, r api.Request[…]) (…) { … }
//
// The custom endpoints keep working through the value inside it, so only what
// you shadow changes.
func New(repo store.FixtureRepository) api.DefaultFixtureService {
	return api.NewFixtureService(repo, &rules{repo: repo})
}

// Bind receives the writer built from the hooks below. rig calls it once,
// during construction.
func (s *rules) Bind(w api.FixtureWriter) { s.write = w }

// Hooks is everything about Fixture that the schema cannot describe, in the
...
// an answer.
func (s *rules) Hooks() api.FixtureHooks {
	return api.FixtureHooks{
		Read: dbhook.ReadHooks[model.FixtureFilter, model.Fixture]{
			Narrow: nil,
			Rows:   nil,
		},
		Create: dbhook.CreateHooks[model.FixtureCreateInput, model.Fixture]{
			Validator: model.FixtureCreateValidator{
				HomeTeamID: s.validateHomeTeamID,
				AwayTeamID: nil,
				KickoffAt:  nil,
				Entity:     nil,
			},
		...
		Update: dbhook.UpdateHooks[model.FixtureUpdateInput, model.Fixture]{
			Validator: model.FixtureUpdateValidator{
				HomeTeamID: s.validateHomeTeamID,
				AwayTeamID: nil,
				KickoffAt:  nil,
				Entity:     nil,
...
// validateHomeTeamID is an example rule. Delete it, and set HomeTeamID back to
// nil in business, if Fixture needs none.
//
// A rule that needs something reaches it through s, which is why the rules are
// methods.
//
// Returning a FieldError attaches the message to home_team_id, so the client
...
func (s *rules) validateHomeTeamID(ctx context.Context, c *model.FixtureValidatorContext, value uuid.UUID) error {
	// c.Values is the whole row as it will be, c.Previous() is how it was, and
	// c.HomeTeamIDChanged() reports whether this request touched it — which is
