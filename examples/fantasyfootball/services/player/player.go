...
// rules is Player's business logic.
//
...
var _ api.PlayerRules = (*rules)(nil)
...
// promoted method:
//
// type Service struct{ api.DefaultPlayerService } func (s *Service) Get(ctx
// context.Context, r api.Request[…]) (…) { … }
//
// The custom endpoints keep working through the value inside it, so only what
// you shadow changes.
func New(repo store.PlayerRepository) api.DefaultPlayerService {
	return api.NewPlayerService(repo, &rules{repo: repo})
}

// Bind receives the writer built from the hooks below. rig calls it once,
// during construction.
func (s *rules) Bind(w api.PlayerWriter) { s.write = w }

// Hooks is everything about Player that the schema cannot describe, in the
...
// an answer.
func (s *rules) Hooks() api.PlayerHooks {
	return api.PlayerHooks{
		...
		Create: dbhook.CreateHooks[model.PlayerCreateInput, model.Player]{
			Validator: model.PlayerCreateValidator{
				FullName:    s.validateFullName,
				Position:    nil,
				ShirtNumber: nil,
				Entity:      nil,
			},
		...
		Update: dbhook.UpdateHooks[model.PlayerUpdateInput, model.Player]{
			Validator: model.PlayerUpdateValidator{
				FullName:    s.validateFullName,
				Position:    nil,
				ShirtNumber: nil,
				Entity:      nil,
...
// validateFullName is an example rule. Delete it, and set FullName back to nil
// in business, if Player needs none.
//
// A rule that needs something reaches it through s, which is why the rules are
// methods.
//
// Returning a FieldError attaches the message to full_name, so the client is
...
func (s *rules) validateFullName(ctx context.Context, c *model.PlayerValidatorContext, value string) error {
	// c.Values is the whole row as it will be, c.Previous() is how it was, and
	// c.FullNameChanged() reports whether this request touched it — which is how
