...
// rules is Team's business logic.
//
...
// promoted method:
//
// type Service struct{ api.DefaultTeamService } func (s *Service) Get(ctx
// context.Context, r api.Request[…]) (…) { … }
//
// The custom endpoints keep working through the value inside it, so only what
// you shadow changes.
func New(repo store.TeamRepository) api.DefaultTeamService {
	return api.NewTeamService(repo, &rules{repo: repo})
}

// Bind receives the writer built from the hooks below. rig calls it once,
// during construction.
func (s *rules) Bind(w api.TeamWriter) { s.write = w }

// Hooks is everything about Team that the schema cannot describe, in the order
...
func (s *rules) Hooks() api.TeamHooks {
	return api.TeamHooks{
		...
		Update: dbhook.UpdateHooks[model.TeamUpdateInput, model.Team]{
			Validator: model.TeamUpdateValidator{
				Name:     s.validateName,
				IsActive: nil,
				Entity:   nil,
			},
...
// validateName is an example rule. Delete it, and set Name back to nil in
// business, if Team needs none.
//
// A rule that needs something reaches it through s, which is why the rules are
// methods.
//
// Returning a FieldError attaches the message to name, so the client is
...
func (s *rules) validateName(ctx context.Context, c *model.TeamValidatorContext, value string) error {
	// c.Values is the whole row as it will be, c.Previous() is how it was, and
	// c.NameChanged() reports whether this request touched it — which is how an
