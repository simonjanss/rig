...
| The bell, and the toasts | the `todo_notification` link table; `NotifyAt`/`NotifyWho` in `api/internal/services/todo/todo.go` (stakeholders minus whoever made the change, resolved at send time); the inbox reaches the browser over the `rig_notification_recipient` shape |
