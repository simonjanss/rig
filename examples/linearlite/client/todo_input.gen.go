// Code generated by rig. DO NOT EDIT.
//
// This file is rewritten on every run. Put changes in the service layer.

package client

import (
	"github.com/google/uuid"
	"github.com/simonjanss/rig/rigclient"
	"github.com/simonjanss/rig/runtime/patch"
	"github.com/simonjanss/rig/runtime/rigerr"
)

// TodoListQuery is the query for Todo.List.
//
// A nil member is left out of the request, and the server applies its own
// default. rigclient.P takes the address of a value, for filling one in.
type TodoListQuery struct {
	// Maximum rows to return. Left out, the server uses 50.
	Limit *int `json:"limit"`
	// Rows to skip before the first returned row. Left out, the server uses 0.
	Offset *int `json:"offset"`
}

// TodoCreateInput is what creating a Todo takes.
//
// A field the server fills in — the identifier, the tenant, the audit
// columns — is not here: those are not a caller's to send.
type TodoCreateInput struct {
	// What needs doing, in a few words.
	Title string `json:"title"`
	// The longer story, or null while the title says it all.
	Description *string `json:"description,omitempty"`
	// Which column of the board the item is in.
	Status TodoStatus `json:"status"`
	// How urgently the item wants attention.
	Priority TodoPriority `json:"priority"`
	// Who has taken the item, or null while nobody has.
	AssigneeAccountID *uuid.UUID `json:"assigneeAccountId,omitempty"`
}

// TodoCreateFields is what a validation failure says, shaped like the body it
// is about — one member per member, so each message can be put beside the
// control it belongs to.
//
// A member is nil when there was nothing wrong with that field. It arrives as
// the Fields of the error the call returns.
type TodoCreateFields struct {
	Title             *rigerr.FieldError `json:"title,omitempty"`
	Description       *rigerr.FieldError `json:"description,omitempty"`
	Status            *rigerr.FieldError `json:"status,omitempty"`
	Priority          *rigerr.FieldError `json:"priority,omitempty"`
	AssigneeAccountID *rigerr.FieldError `json:"assigneeAccountId,omitempty"`
	// Entity carries what was wrong with the request as a whole rather than with
	// any one field.
	Entity *rigerr.FieldError `json:"entity,omitempty"`
}

// TodoCreateError reads back what the server said about a refused
// Todos.Create: the envelope — Code, Message, RequestID, Status, RetryAfter
// — and, when the refusal was about the body, Fields shaped like the body
// that failed.
//
//	if refused, ok := TodoCreateError(err); ok {
//		if refused.Fields != nil && refused.Fields.Title != nil {
//
// Fields is nil for every refusal but a 422: a 404 has a code and a message
// and nothing to put beside a control. The second value is false for anything
// that is not a refusal at all, which is where a request that never reached
// the server ends up.
func TodoCreateError(err error) (*rigclient.Failure[TodoCreateFields], bool) {
	return rigclient.As[TodoCreateFields](err)
}

// TodoSearchBody is the request body for Todo.Search.
type TodoSearchBody struct {
	// Conditions rows must satisfy.
	Filter TodoFilter `json:"filter"`
}

// TodoSearchQuery is the query for Todo.Search.
//
// A nil member is left out of the request, and the server applies its own
// default. rigclient.P takes the address of a value, for filling one in.
type TodoSearchQuery struct {
	// Maximum rows to return. Left out, the server uses 50.
	Limit *int `json:"limit"`
	// Rows to skip before the first returned row. Left out, the server uses 0.
	Offset *int `json:"offset"`
}

// TodoListDeletedQuery is the query for Todo.ListDeleted.
//
// A nil member is left out of the request, and the server applies its own
// default. rigclient.P takes the address of a value, for filling one in.
type TodoListDeletedQuery struct {
	// Maximum rows to return. Left out, the server uses 50.
	Limit *int `json:"limit"`
	// Rows to skip before the first returned row. Left out, the server uses 0.
	Offset *int `json:"offset"`
}

// TodoUpdateInput is what changing a Todo takes.
//
// A field left out is left alone. A nullable field set to patch.Null is
// cleared — which is why there are two wrappers: a column that cannot hold
// null has no way to be given one, so clearing it is a compile error rather
// than a refusal at runtime. Immutable fields are not here at all.
type TodoUpdateInput struct {
	// What needs doing, in a few words.
	Title patch.Optional[string] `json:"title,omitzero"`
	// The longer story, or null while the title says it all.
	Description patch.Nullable[string] `json:"description,omitzero"`
	// Which column of the board the item is in.
	Status patch.Optional[TodoStatus] `json:"status,omitzero"`
	// How urgently the item wants attention.
	Priority patch.Optional[TodoPriority] `json:"priority,omitzero"`
	// Who has taken the item, or null while nobody has.
	AssigneeAccountID patch.Nullable[uuid.UUID] `json:"assigneeAccountId,omitzero"`
}

// TodoUpdateFields is what a validation failure says, shaped like the body it
// is about — one member per member, so each message can be put beside the
// control it belongs to.
//
// A member is nil when there was nothing wrong with that field. It arrives as
// the Fields of the error the call returns.
type TodoUpdateFields struct {
	Title             *rigerr.FieldError `json:"title,omitempty"`
	Description       *rigerr.FieldError `json:"description,omitempty"`
	Status            *rigerr.FieldError `json:"status,omitempty"`
	Priority          *rigerr.FieldError `json:"priority,omitempty"`
	AssigneeAccountID *rigerr.FieldError `json:"assigneeAccountId,omitempty"`
	// Entity carries what was wrong with the request as a whole rather than with
	// any one field.
	Entity *rigerr.FieldError `json:"entity,omitempty"`
}

// TodoUpdateError reads back what the server said about a refused
// Todos.Update: the envelope — Code, Message, RequestID, Status, RetryAfter
// — and, when the refusal was about the body, Fields shaped like the body
// that failed.
//
//	if refused, ok := TodoUpdateError(err); ok {
//		if refused.Fields != nil && refused.Fields.Title != nil {
//
// Fields is nil for every refusal but a 422: a 404 has a code and a message
// and nothing to put beside a control. The second value is false for anything
// that is not a refusal at all, which is where a request that never reached
// the server ends up.
func TodoUpdateError(err error) (*rigclient.Failure[TodoUpdateFields], bool) {
	return rigclient.As[TodoUpdateFields](err)
}

// TodoClaimBody is the request body for Todo.Claim.
type TodoClaimBody struct {
	// Take it even if somebody else holds it.
	Steal *bool `json:"steal,omitempty"`
}

// TodoClaimFields is what a validation failure says, shaped like the body it
// is about — one member per member, so each message can be put beside the
// control it belongs to.
//
// A member is nil when there was nothing wrong with that field. It arrives as
// the Fields of the error the call returns.
type TodoClaimFields struct {
	Steal *rigerr.FieldError `json:"steal,omitempty"`
	// Entity carries what was wrong with the request as a whole rather than with
	// any one field.
	Entity *rigerr.FieldError `json:"entity,omitempty"`
}

// TodoClaimError reads back what the server said about a refused Todos.Claim:
// the envelope — Code, Message, RequestID, Status, RetryAfter — and, when
// the refusal was about the body, Fields shaped like the body that failed.
//
//	if refused, ok := TodoClaimError(err); ok {
//		if refused.Fields != nil && refused.Fields.Steal != nil {
//
// Fields is nil for every refusal but a 422: a 404 has a code and a message
// and nothing to put beside a control. The second value is false for anything
// that is not a refusal at all, which is where a request that never reached
// the server ends up.
func TodoClaimError(err error) (*rigclient.Failure[TodoClaimFields], bool) {
	return rigclient.As[TodoClaimFields](err)
}

// TodoRevertBody is the request body for Todo.Revert.
type TodoRevertBody struct {
	// The version to put back, from the Todo's history.
	VersionID uuid.UUID `json:"versionId"`
}
