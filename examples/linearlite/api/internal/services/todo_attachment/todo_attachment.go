...
func (s *rules) Hooks() api.TodoAttachmentHooks {
	return api.TodoAttachmentHooks{
		...
		Parents: api.TodoAttachmentParentHooks{
			// What should happen to this table's rows when the Todo that todo_id points at
			// is deleted. Nil refuses the delete, because the foreign key does.
			//
			// Two shapes, and they do not look different. `s.repo.ClearTodo(ctx,
			// parent.ID)` is one statement and skips this table's own hooks and snapshots.
			// A loop calling this service's own Delete on each row gets both, and costs a
			// statement per row. Which one is right is a question about this table, and it
			// is the reason this is a function.
			TodoDeleting: nil,
			TodoDeleted:  nil,
		},
	}
