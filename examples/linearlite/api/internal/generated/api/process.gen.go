...
//
// The fields are Traces, Notifications, Presence, Shapes and Auth, and there
// are no others because those are the steps this project's configuration
// registers. That is the whole reason this is generated: a step this server
// does not have is one there is no way to write a number for, and
...
type Shutdown struct {
	...
	// The presence sweeper, 5 * time.Second as generated.
...
func (s Shutdown) Steps() []serve.Step {
	...
	if s.Presence > 0 {
		steps = append(steps, serve.Step{Name: "presence", Timeout: s.Presence})
	}
	if s.Shapes > 0 {
...
func (s Shutdown) Budget() time.Duration {
	return cmp.Or(s.Traces, tracesShutdown) + cmp.Or(s.Notifications, notificationsShutdown) + cmp.Or(s.Presence, presenceShutdown) + cmp.Or(s.Shapes, shapesShutdown) + cmp.Or(s.Auth, authShutdown) + shutdownHeadroom
}
