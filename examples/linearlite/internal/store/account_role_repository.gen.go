...
// accountRoleAccountFilter collects the conditions written on a AccountRole's
// Account.
//
...
func accountRoleAccountFilter(f model.AccountRoleFilter) (model.RigAccountFilter, bool) {
	var (
...
func accountRoleGroup(f model.AccountRoleFilter, sc filterScope) (query.Group, error) {
	...
	if sub, ok := accountRoleAccountFilter(f); ok {
...
// accountRoleOrderColumn reports whether an ordering names a column of a
// relation that can be ordered by.
func accountRoleOrderColumn(relation, column string) error {
	switch relation {
	case "Account":
		if !rigAccountSortable(column) {
			return rigerr.Invalid("a AccountRole has no Account that can be ordered by %q", column)
		}
		return nil
	}
	return rigerr.Invalid("a AccountRole has no relation named %q to order by", relation)
}

// accountRoleOrderJoin builds the left join an ordering across a relation
// needs.
//
...
// clothes.
func accountRoleOrderJoin(relation, column, alias string, sc filterScope) (query.Join, error) {
	if err := accountRoleOrderColumn(relation, column); err != nil {
	...
	return query.Join{}, rigerr.Invalid("a AccountRole has no relation named %q to order by", relation)
}
	...
	for _, t := range terms {
		if t.Relation != "" {
			...
			if !ok {
				...
				alias = "o" + alias
				join, err := accountRoleOrderJoin(t.Relation, t.Column, alias, sc)
				if err != nil {
					return nil, nil, err
				}
				joins = append(joins, join)
				aliases[t.Relation] = alias
			} else if err := accountRoleOrderColumn(t.Relation, t.Column); err != nil {
				return nil, nil, err
			}
...
func (r *accountRoleRepo) list(ctx context.Context, f model.AccountRoleFilter, page model.AccountRolePage, opts []readopt.Option) ([]*model.AccountRole, int64, error) {
	...
	sc := newFilterScope(claims, "account_role")
...
func (r *accountRoleRepo) Create(ctx context.Context, in dbhook.Create[model.AccountRoleCreateInput, model.AccountRole]) (*model.AccountRole, error) {
	...
	err = dbx.InTxIf(ctx, r.db.pool, r.db.connFor(ctx), needsTx, func(ctx context.Context, tx dbx.Conn) error {
		...
		// to do the write.
		{
			...
			if !ok {
				return &model.AccountRoleCreateInputError{AccountID: rigerr.NewFieldError(rigerr.FieldCodeNotFound, "no RigAccount with id %s", in.Input.AccountID)}
			}
		}
...
func (r *accountRoleRepo) Update(ctx context.Context, id uuid.UUID, in dbhook.Update[model.AccountRoleUpdateInput, model.AccountRole]) (*model.AccountRole, error) {
	...
	err = dbx.InTx(ctx, r.db.pool, func(ctx context.Context, tx dbx.Conn) error {
		...
		if v, sent := in.Input.AccountID.Get(); sent {
			...
			if !ok {
				return &model.AccountRoleUpdateInputError{AccountID: rigerr.NewFieldError(rigerr.FieldCodeNotFound, "no RigAccount with id %s", v)}
			}
		}
