...
func todoAttachmentSortable(column string) bool {
	switch column {
	case "id", "tenant_id", "todo_id", "attachment_file_id", "caption", "created_at", "created_by_account_id", "updated_at", "updated_by_account_id", "deleted_at", "deleted_by_account_id":
		return true
	}
	return false
}
...
func scanTodoAttachment(row pgx.Row) (*model.TodoAttachment, error) {
	...
	if err := row.Scan(&m.ID, &m.TenantID, &m.TodoID, &m.AttachmentFileID, &m.Caption, &m.CreatedAt, &m.CreatedByAccountID, &m.UpdatedAt, &m.UpdatedByAccountID, &m.DeletedAt, &m.DeletedByAccountID); err != nil {
		return nil, err
	}
...
func (r *todoAttachmentRepo) Create(ctx context.Context, in dbhook.Create[model.TodoAttachmentCreateInput, model.TodoAttachment]) (*model.TodoAttachment, error) {
	...
	err = dbx.InTxIf(ctx, r.db.pool, r.db.connFor(ctx), needsTx, func(ctx context.Context, tx dbx.Conn) error {
		...
		columns := []string{"id", "tenant_id", "created_at", "created_by_account_id", "todo_id", "attachment_file_id", "caption"}
		values := []any{id, claims.TenantID, now, claims.Actor(), in.Input.TodoID, in.Input.AttachmentFileID, in.Input.Caption}
