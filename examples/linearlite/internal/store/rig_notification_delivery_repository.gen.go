...
// rigNotificationDeliveryAccountFilter collects the conditions written on a
// RigNotificationDelivery's Account.
//
...
func rigNotificationDeliveryAccountFilter(f model.RigNotificationDeliveryFilter) (model.RigAccountFilter, bool) {
	var (
...
func rigNotificationDeliveryGroup(f model.RigNotificationDeliveryFilter, sc filterScope) (query.Group, error) {
	...
	if sub, ok := rigNotificationDeliveryAccountFilter(f); ok {
...
// rigNotificationDeliveryOrderColumn reports whether an ordering names a
// column of a relation that can be ordered by.
func rigNotificationDeliveryOrderColumn(relation, column string) error {
	switch relation {
	case "Account":
		if !rigAccountSortable(column) {
			return rigerr.Invalid("a RigNotificationDelivery has no Account that can be ordered by %q", column)
		}
		return nil
	}
	return rigerr.Invalid("a RigNotificationDelivery has no relation named %q to order by", relation)
}

// rigNotificationDeliveryOrderJoin builds the left join an ordering across a
// relation needs.
//
...
// clothes.
func rigNotificationDeliveryOrderJoin(relation, column, alias string, sc filterScope) (query.Join, error) {
	if err := rigNotificationDeliveryOrderColumn(relation, column); err != nil {
	...
	return query.Join{}, rigerr.Invalid("a RigNotificationDelivery has no relation named %q to order by", relation)
}
	...
	for _, t := range terms {
		if t.Relation != "" {
			...
			if !ok {
				...
				alias = "o" + alias
				join, err := rigNotificationDeliveryOrderJoin(t.Relation, t.Column, alias, sc)
				if err != nil {
					return nil, nil, err
				}
				joins = append(joins, join)
				aliases[t.Relation] = alias
			} else if err := rigNotificationDeliveryOrderColumn(t.Relation, t.Column); err != nil {
				return nil, nil, err
			}
...
func (r *rigNotificationDeliveryRepo) Create(ctx context.Context, in dbhook.Create[model.RigNotificationDeliveryCreateInput, model.RigNotificationDelivery]) (*model.RigNotificationDelivery, error) {
	...
	err = dbx.InTxIf(ctx, r.db.pool, r.db.connFor(ctx), needsTx, func(ctx context.Context, tx dbx.Conn) error {
		...
		// to do the write.
		{
			...
			if !ok {
				return &model.RigNotificationDeliveryCreateInputError{AccountID: rigerr.NewFieldError(rigerr.FieldCodeNotFound, "no RigAccount with id %s", in.Input.AccountID)}
			}
		}
...
func (r *rigNotificationDeliveryRepo) Update(ctx context.Context, id uuid.UUID, in dbhook.Update[model.RigNotificationDeliveryUpdateInput, model.RigNotificationDelivery]) (*model.RigNotificationDelivery, error) {
	...
	err = dbx.InTx(ctx, r.db.pool, func(ctx context.Context, tx dbx.Conn) error {
		...
		if v, sent := in.Input.AccountID.Get(); sent {
			...
			if !ok {
				return &model.RigNotificationDeliveryUpdateInputError{AccountID: rigerr.NewFieldError(rigerr.FieldCodeNotFound, "no RigAccount with id %s", v)}
			}
		}
