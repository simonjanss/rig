...
// PermissionRepository reads and writes permission rows.
//
...
type PermissionRepository interface {
	...
	Get(ctx context.Context, id uuid.UUID, opts ...readopt.Option) (*model.Permission, error)
	...
	List(ctx context.Context, f model.PermissionFilter, page model.PermissionPage, opts ...readopt.Option) ([]*model.Permission, int64, error)
	...
	Create(ctx context.Context, in dbhook.Create[model.PermissionCreateInput, model.Permission]) (*model.Permission, error)

	// Update changes the fields the input mentions and leaves the rest alone.
	Update(ctx context.Context, id uuid.UUID, in dbhook.Update[model.PermissionUpdateInput, model.Permission]) (*model.Permission, error)

	// Delete removes a row.
	Delete(ctx context.Context, in dbhook.Delete[model.PermissionDeleteInput, model.Permission]) error
}

// permissionGroup turns a filter into the condition tree the runtime renders.
//
...
func permissionGroup(f model.PermissionFilter, sc filterScope) (query.Group, error) {
	...
	for _, n := range f.NestedFilters {
		nested, err := permissionGroup(n, sc)
		if err != nil {
			return query.Group{}, err
		}
		g.Nest(nested)
...
// permissionSortable reports whether a column can be ordered by.
func permissionSortable(column string) bool {
	switch column {
	case "id", "created_at", "key", "name", "description":
		return true
	}
	return false
}

// permissionOrder converts the model's ordering terms into the runtime's.
func permissionOrder(terms []model.PermissionOrder, sc filterScope) ([]query.Order, []query.Join, error) {
	if len(terms) == 0 {
		return nil, nil, nil
	}
	...
	for _, t := range terms {
		...
		if !permissionSortable(t.Column) {
			return nil, nil, rigerr.Invalid("a Permission cannot be ordered by %q", t.Column)
		}
...
func scanPermission(row pgx.Row) (*model.Permission, error) {
	...
	if err := row.Scan(&m.ID, &m.CreatedAt, &m.Key, &m.Name, &m.Description); err != nil {
		return nil, err
	}
...
func (r *permissionRepo) Get(ctx context.Context, id uuid.UUID, opts ...readopt.Option) (*model.Permission, error) {
	...
	if err != nil {
		return nil, rigerr.Internal(err, "read permission")
	}
	return m, nil
}
...
func (r *permissionRepo) list(ctx context.Context, f model.PermissionFilter, page model.PermissionPage, opts []readopt.Option) ([]*model.Permission, int64, error) {
	...
	sc := newFilterScope(tenancy.Claims{}, "permission")

	scope := query.Group{}

	group, err := permissionGroup(f, sc)
	if err != nil {
		return nil, 0, err
	}
	scope.Nest(group)
	...
	order, joins, err := permissionOrder(page.OrderBy, sc)
	if err != nil {
		return nil, 0, err
	}
	if len(order) == 0 {
		order = PermissionDefaultOrder
	}
	...
	out := make([]*model.Permission, 0, window.Limit)
	for rows.Next() {
		m, err := scanPermission(rows)
		if err != nil {
			return nil, 0, rigerr.Internal(err, "read permission")
		}
		out = append(out, m)
	}
...
// PermissionDefaultOrder is the ordering used when a query asks for none.
//
// It always ends with the primary key, so the order is total and a page
// boundary cannot repeat or skip a row.
var PermissionDefaultOrder = []query.Order{{Table: "permission", Column: "created_at", Desc: true}, {Table: "permission", Column: "id", Desc: false}}
...
func (r *permissionRepo) Create(ctx context.Context, in dbhook.Create[model.PermissionCreateInput, model.Permission]) (*model.Permission, error) {
	...
	err = dbx.InTxIf(ctx, r.db.pool, r.db.connFor(ctx), needsTx, func(ctx context.Context, tx dbx.Conn) error {
		...
		columns := []string{"id", "created_at", "key", "name", "description"}
		values := []any{id, now, in.Input.Key, in.Input.Name, in.Input.Description}

		sql := fmt.Sprintf("INSERT INTO permission (%s) VALUES (%s) RETURNING %s", joinColumns(columns), placeholders(len(values)), permissionRepoSelect)

		created, err := scanPermission(tx.QueryRow(ctx, sql, values...))
		if err != nil {
...
func (r *permissionRepo) Update(ctx context.Context, id uuid.UUID, in dbhook.Update[model.PermissionUpdateInput, model.Permission]) (*model.Permission, error) {
	...
	var updated, prev *model.Permission
	err = dbx.InTx(ctx, r.db.pool, func(ctx context.Context, tx dbx.Conn) error {
		...
		sql := fmt.Sprintf("UPDATE permission SET %s WHERE id = $%d RETURNING %s", assignments(columns), len(values), permissionRepoSelect)

		updated, err = scanPermission(tx.QueryRow(ctx, sql, values...))
		if err != nil {
			return writeError(err, "permission")
		}
...
func (r *permissionRepo) Delete(ctx context.Context, in dbhook.Delete[model.PermissionDeleteInput, model.Permission]) error {
	...
	err = dbx.InTx(ctx, r.db.pool, func(ctx context.Context, tx dbx.Conn) error {
		...
		if _, err := tx.Exec(ctx, "DELETE FROM permission WHERE id = $1", in.Input.ID); err != nil {
			return writeError(err, "permission")
		}
		if in.Hooks.After != nil {
