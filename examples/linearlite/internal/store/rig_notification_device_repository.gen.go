...
// rigNotificationDeviceAccountFilter collects the conditions written on a
// RigNotificationDevice's Account.
//
...
func rigNotificationDeviceAccountFilter(f model.RigNotificationDeviceFilter) (model.RigAccountFilter, bool) {
	var (
...
func rigNotificationDeviceGroup(f model.RigNotificationDeviceFilter, sc filterScope) (query.Group, error) {
	...
	if sub, ok := rigNotificationDeviceAccountFilter(f); ok {
...
// rigNotificationDeviceOrderColumn reports whether an ordering names a column
// of a relation that can be ordered by.
func rigNotificationDeviceOrderColumn(relation, column string) error {
	switch relation {
	case "Account":
		if !rigAccountSortable(column) {
			return rigerr.Invalid("a RigNotificationDevice has no Account that can be ordered by %q", column)
		}
		return nil
	}
	return rigerr.Invalid("a RigNotificationDevice has no relation named %q to order by", relation)
}

// rigNotificationDeviceOrderJoin builds the left join an ordering across a
// relation needs.
//
...
// clothes.
func rigNotificationDeviceOrderJoin(relation, column, alias string, sc filterScope) (query.Join, error) {
	if err := rigNotificationDeviceOrderColumn(relation, column); err != nil {
	...
	return query.Join{}, rigerr.Invalid("a RigNotificationDevice has no relation named %q to order by", relation)
}
	...
	for _, t := range terms {
		if t.Relation != "" {
			...
			if !ok {
				...
				alias = "o" + alias
				join, err := rigNotificationDeviceOrderJoin(t.Relation, t.Column, alias, sc)
				if err != nil {
					return nil, nil, err
				}
				joins = append(joins, join)
				aliases[t.Relation] = alias
			} else if err := rigNotificationDeviceOrderColumn(t.Relation, t.Column); err != nil {
				return nil, nil, err
			}
...
func (r *rigNotificationDeviceRepo) Create(ctx context.Context, in dbhook.Create[model.RigNotificationDeviceCreateInput, model.RigNotificationDevice]) (*model.RigNotificationDevice, error) {
	...
	err = dbx.InTxIf(ctx, r.db.pool, r.db.connFor(ctx), needsTx, func(ctx context.Context, tx dbx.Conn) error {
		...
		// to do the write.
		{
			...
			if !ok {
				return &model.RigNotificationDeviceCreateInputError{AccountID: rigerr.NewFieldError(rigerr.FieldCodeNotFound, "no RigAccount with id %s", in.Input.AccountID)}
			}
		}
...
func (r *rigNotificationDeviceRepo) Update(ctx context.Context, id uuid.UUID, in dbhook.Update[model.RigNotificationDeviceUpdateInput, model.RigNotificationDevice]) (*model.RigNotificationDevice, error) {
	...
	err = dbx.InTx(ctx, r.db.pool, func(ctx context.Context, tx dbx.Conn) error {
		...
		if v, sent := in.Input.AccountID.Get(); sent {
			...
			if !ok {
				return &model.RigNotificationDeviceUpdateInputError{AccountID: rigerr.NewFieldError(rigerr.FieldCodeNotFound, "no RigAccount with id %s", v)}
			}
		}
