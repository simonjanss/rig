...
// NotificationDeviceRepository reads and writes rig_notification_device rows.
//
...
type NotificationDeviceRepository interface {
	...
	Get(ctx context.Context, id uuid.UUID, opts ...readopt.Option) (*model.NotificationDevice, error)
	...
	List(ctx context.Context, f model.NotificationDeviceFilter, page model.NotificationDevicePage, opts ...readopt.Option) ([]*model.NotificationDevice, int64, error)
	...
	Create(ctx context.Context, in dbhook.Create[model.NotificationDeviceCreateInput, model.NotificationDevice]) (*model.NotificationDevice, error)

	// Update changes the fields the input mentions and leaves the rest alone.
	Update(ctx context.Context, id uuid.UUID, in dbhook.Update[model.NotificationDeviceUpdateInput, model.NotificationDevice]) (*model.NotificationDevice, error)

	// Delete removes a row.
	Delete(ctx context.Context, in dbhook.Delete[model.NotificationDeviceDeleteInput, model.NotificationDevice]) error
}

// notificationDeviceGroup turns a filter into the condition tree the runtime
// renders.
//
...
func notificationDeviceGroup(f model.NotificationDeviceFilter, sc filterScope) (query.Group, error) {
	...
	for _, n := range f.NestedFilters {
		nested, err := notificationDeviceGroup(n, sc)
		if err != nil {
			return query.Group{}, err
		}
		g.Nest(nested)
...
// notificationDeviceSortable reports whether a column can be ordered by.
func notificationDeviceSortable(column string) bool {
	switch column {
...
// notificationDeviceOrder converts the model's ordering terms into the
// runtime's.
func notificationDeviceOrder(terms []model.NotificationDeviceOrder, sc filterScope) ([]query.Order, []query.Join, error) {
	if len(terms) == 0 {
		return nil, nil, nil
	}
	...
	for _, t := range terms {
		...
		if !notificationDeviceSortable(t.Column) {
			return nil, nil, rigerr.Invalid("a NotificationDevice cannot be ordered by %q", t.Column)
		}
...
type notificationDeviceRepo struct {
	db *Store
}

var _ NotificationDeviceRepository = (*notificationDeviceRepo)(nil)
...
func (r *notificationDeviceRepo) trace(ctx context.Context, name string, f func(context.Context) error) error {
	return observe.Trace(ctx, r.db.tracer, name, f)
}

const notificationDeviceRepoSelect = "rig_notification_device.id, rig_notification_device.tenant_id, rig_notification_device.account_id, rig_notification_device.created_at, rig_notification_device.updated_at, rig_notification_device.channel, rig_notification_device.token, rig_notification_device.label, rig_notification_device.last_seen_at, rig_notification_device.revoked_at"

// scanNotificationDevice reads one row in the order
// notificationDeviceRepoSelect lists.
func scanNotificationDevice(row pgx.Row) (*model.NotificationDevice, error) {
	var m model.NotificationDevice
...
// Get implements NotificationDeviceRepository.
func (r *notificationDeviceRepo) Get(ctx context.Context, id uuid.UUID, opts ...readopt.Option) (*model.NotificationDevice, error) {
	ctx, span := r.db.tracer.Start(ctx, "repository.NotificationDevice.Get")
	defer span.End()
	...
	sql := fmt.Sprintf("SELECT %s FROM rig_notification_device WHERE %s", notificationDeviceRepoSelect, where)
	m, err := scanNotificationDevice(r.db.connFor(ctx).QueryRow(ctx, sql, args...))
	if dbx.IsNoRows(err) {
		return nil, rigerr.NotFound("no NotificationDevice with id %s", id)
	}
	if err != nil {
...
// List implements NotificationDeviceRepository.
func (r *notificationDeviceRepo) List(ctx context.Context, f model.NotificationDeviceFilter, page model.NotificationDevicePage, opts ...readopt.Option) ([]*model.NotificationDevice, int64, error) {
	ctx, span := r.db.tracer.Start(ctx, "repository.NotificationDevice.List")
	defer span.End()
...
func (r *notificationDeviceRepo) list(ctx context.Context, f model.NotificationDeviceFilter, page model.NotificationDevicePage, opts []readopt.Option) ([]*model.NotificationDevice, int64, error) {
	...
	group, err := notificationDeviceGroup(f, sc)
	if err != nil {
		return nil, 0, err
	}
	scope.Nest(group)
	...
	order, joins, err := notificationDeviceOrder(page.OrderBy, sc)
	if err != nil {
		return nil, 0, err
	}
	if len(order) == 0 {
		order = NotificationDeviceDefaultOrder
	}
	...
	listSQL := fmt.Sprintf("SELECT %s FROM rig_notification_device%s%s%s%s", notificationDeviceRepoSelect, joinSQL, where, query.OrderSQL(order), window.SQL(args))
	...
	out := make([]*model.NotificationDevice, 0, window.Limit)
	for rows.Next() {
		m, err := scanNotificationDevice(rows)
		if err != nil {
...
// NotificationDeviceDefaultOrder is the ordering used when a query asks for
// none.
//
// It always ends with the primary key, so the order is total and a page
// boundary cannot repeat or skip a row.
var NotificationDeviceDefaultOrder = []query.Order{{Table: "rig_notification_device", Column: "created_at", Desc: true}, {Table: "rig_notification_device", Column: "id", Desc: false}}

// Create implements NotificationDeviceRepository.
func (r *notificationDeviceRepo) Create(ctx context.Context, in dbhook.Create[model.NotificationDeviceCreateInput, model.NotificationDevice]) (*model.NotificationDevice, error) {
	ctx, span := r.db.tracer.Start(ctx, "repository.NotificationDevice.Create")
	defer span.End()
	...
	if in.Hooks.Validator != nil {
		if err := r.trace(ctx, "repository.NotificationDevice.Create.Validator", func(ctx context.Context) error {
	...
	err = dbx.InTxIf(ctx, r.db.pool, r.db.connFor(ctx), needsTx, func(ctx context.Context, tx dbx.Conn) error {
		if in.Hooks.Before != nil {
			if err := r.trace(ctx, "repository.NotificationDevice.Create.Before", func(ctx context.Context) error {
		...
		sql := fmt.Sprintf("INSERT INTO rig_notification_device (%s) VALUES (%s) RETURNING %s", joinColumns(columns), placeholders(len(values)), notificationDeviceRepoSelect)

		created, err := scanNotificationDevice(tx.QueryRow(ctx, sql, values...))
		if err != nil {
		...
		if in.Hooks.After != nil {
			if err := r.trace(ctx, "repository.NotificationDevice.Create.After", func(ctx context.Context) error {
	...
	if in.Hooks.AfterCommit != nil {
		...
		dbx.AfterCommit(ctx, func() {
			ctx, span := r.db.tracer.Start(ctx, "repository.NotificationDevice.Create.AfterCommit")
			defer span.End()
...
// Update implements NotificationDeviceRepository.
func (r *notificationDeviceRepo) Update(ctx context.Context, id uuid.UUID, in dbhook.Update[model.NotificationDeviceUpdateInput, model.NotificationDevice]) (*model.NotificationDevice, error) {
	ctx, span := r.db.tracer.Start(ctx, "repository.NotificationDevice.Update")
	defer span.End()
	...
	var updated, prev *model.NotificationDevice
	err = dbx.InTx(ctx, r.db.pool, func(ctx context.Context, tx dbx.Conn) error {
		...
		// checked.
		if in.Hooks.Before != nil {
			if err := r.trace(ctx, "repository.NotificationDevice.Update.Before", func(ctx context.Context) error {
		...
		if in.Hooks.Validator != nil {
			if err := r.trace(ctx, "repository.NotificationDevice.Update.Validator", func(ctx context.Context) error {
		...
		sql := fmt.Sprintf("UPDATE rig_notification_device SET %s WHERE id = $%d RETURNING %s", assignments(columns), len(values), notificationDeviceRepoSelect)

		updated, err = scanNotificationDevice(tx.QueryRow(ctx, sql, values...))
		if err != nil {
			return writeError(err, "rig_notification_device")
		}

		if in.Hooks.After != nil {
			if err := r.trace(ctx, "repository.NotificationDevice.Update.After", func(ctx context.Context) error {
	...
	if in.Hooks.AfterCommit != nil {
		...
		dbx.AfterCommit(ctx, func() {
			ctx, span := r.db.tracer.Start(ctx, "repository.NotificationDevice.Update.AfterCommit")
			defer span.End()
...
// Delete implements NotificationDeviceRepository.
func (r *notificationDeviceRepo) Delete(ctx context.Context, in dbhook.Delete[model.NotificationDeviceDeleteInput, model.NotificationDevice]) error {
	ctx, span := r.db.tracer.Start(ctx, "repository.NotificationDevice.Delete")
	defer span.End()
	...
	var prev *model.NotificationDevice
	err = dbx.InTx(ctx, r.db.pool, func(ctx context.Context, tx dbx.Conn) error {
		...
		if in.Hooks.Before != nil {
			if err := r.trace(ctx, "repository.NotificationDevice.Delete.Before", func(ctx context.Context) error {
				return in.Hooks.Before(ctx, claims, &in.Input, prev)
			}); err != nil {
				return err
			}
		...
		if in.Hooks.After != nil {
			if err := r.trace(ctx, "repository.NotificationDevice.Delete.After", func(ctx context.Context) error {
	...
	if in.Hooks.AfterCommit != nil {
		...
		dbx.AfterCommit(ctx, func() {
			ctx, span := r.db.tracer.Start(ctx, "repository.NotificationDevice.Delete.AfterCommit")
			defer span.End()
