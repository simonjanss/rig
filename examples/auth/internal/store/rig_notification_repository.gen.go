...
func rigNotificationSortable(column string) bool {
	switch column {
	case "id", "tenant_id", "created_at", "created_by_account_id", "created_by_api_key_id", "updated_at", "kind", "state", "deliver_at", "resolved_at", "payload", "group_key":
		return true
	}
	return false
}
...
func scanRigNotification(row pgx.Row) (*model.RigNotification, error) {
	...
	if err := row.Scan(&m.ID, &m.TenantID, &m.CreatedAt, &m.CreatedByAccountID, &m.CreatedByAPIKeyID, &m.UpdatedAt, &m.Kind, &m.State, &m.DeliverAt, &m.ResolvedAt, &m.Payload, &m.GroupKey, &m.AccountIds); err != nil {
		return nil, err
	}
...
func (r *rigNotificationRepo) Create(ctx context.Context, in dbhook.Create[model.RigNotificationCreateInput, model.RigNotification]) (*model.RigNotification, error) {
	...
	err = dbx.InTxIf(ctx, r.db.pool, r.db.conn(), needsTx, func(ctx context.Context, tx dbx.Conn) error {
		...
		columns := []string{"id", "tenant_id", "created_at", "created_by_account_id", "created_by_api_key_id", "kind", "state", "deliver_at", "resolved_at", "payload", "group_key", "account_ids"}
		values := []any{id, claims.TenantID, now, claims.Actor(), claims.ActorKey(), in.Input.Kind, in.Input.State, in.Input.DeliverAt, in.Input.ResolvedAt, in.Input.Payload, in.Input.GroupKey, in.Input.AccountIds}
