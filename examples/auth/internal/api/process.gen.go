...
//
// For this project that is 35s: 15s for the notification engine, 5s for the
...
//
//	MaxShutdown: 40 * time.Second // 35s here, plus 5s for a closer of my own
//
...
func ShutdownBudget() time.Duration {
	return notificationsShutdown + shapesShutdown + authShutdown + shutdownHeadroom
}
...
//
// The fields are Notifications, Shapes and Auth, and there are no others
// because those are the steps this project's configuration registers. That is
// the whole reason this is generated: a step this server does not have is one
