...
//
//	go test -tags docker ./migrate/
//
// It is ~a hundred lines internal/dockerdb already has, and the duplication is
// deliberate for the reason rigtest/postgres_docker_test.go and
// rigs3/minio_docker_test.go both state: that package lives under the CLI's
// module, and a published module that imported it would require the whole of
// rig to build. Here it is not only a rule — internal/dockerdb imports this
// module, so the import would be a cycle. So this borrows the two ideas it
// cannot do without and nothing else: a container name qualified by a digest of
...
//
// This suite used to expect a database somebody else had started, on a port
// written into it. That cannot work under isolation, where the port is the
// kernel's to choose and no number is knowable in advance: what it produced was
// a suite that skipped under `make` and a concurrency test that reported the
// advisory lock broken because its runners had nothing to connect to. Starting
// its own is what makes a green run here mean it ran.
//
// The number in pgPort is dockerdb.PortMigrate, written out because this module
...
const (
	...
	pgName  = "migrate-db"
	// pgPort is dockerdb.PortMigrate. See this file's doc comment.
	pgPort    = 55448
