...
`examples/linearlite/api/internal/services/outbox` is a worked one — the shape a
real sender has and none of the substance, recording what it was handed instead
of sending it, so the example can show the mail beside the bell. It is the same
type that implements `account.Notifier` for the auth package's links, which is
worth noticing: "somebody was told something" has two sources and one
destination.
