...
func (i *NotificationSettingCreateInput) Normalize() {
	...
	// The column's DEFAULT, applied here because a create carries no way to say
	// "unset". It is why a create cannot make a disabled setting: the zero value
	// of a bool and an explicit false are the same value, so this turns both
	// into true. PATCH can, because there IsEnabled is a patch.Optional.
	if !i.IsEnabled {
		i.IsEnabled = true
	}
