...
// row is not.
func (e *emitter) aliasFile(res *ir.Resource, importPath, shipped string) (gen.Artifact, error) {
	b := gobuf.New(e.pkg)
	...
	b.Comment(res.Description + "\n\n" +
		...
		"[" + importPath + "." + shipped + "], not a second struct of the same " +
	...
	b.L("type %s = %s.%s", res.Name, pkg, shipped)
	b.NL()
	...
	for _, suffix := range []string{
		...
		b.L("%s%s = %s.%s%s", res.Name, suffix, pkg, shipped, suffix)
	}
	b.L(")")
	b.NL()

	b.Comment("Table" + res.Name + " is the table this entity is stored in.")
	b.L("const Table%s = %s.Table%s", res.Name, pkg, shipped)
	b.NL()

	b.Comment("Column names for " + res.Storage.Table + ", so nothing has to spell one out.")
	b.L("const (")
	for i := range t.Columns {
		col := n.Go(t.Columns[i].Name)
		b.L("Column%s%s = %s.Column%s%s", res.Name, col, pkg, shipped, col)
	}
	b.L(")")
	b.NL()

	b.Comment(res.Name + "Columns is every column, in the order the row is scanned.")
	b.L("var %sColumns = %s.%sColumns", res.Name, pkg, shipped)
	b.NL()
...
func (e *emitter) enumAliasFile(enum ir.Enum, importPath, shipped string) (gen.Artifact, error) {
	...
	b.Comment(enum.Description + "\n\n" +
		"An alias for [" + importPath + "." + shipped + "]: rig's schema " +
		"declared the type, so rig declares the Go for it.")
	b.L("type %s = %s.%s", enum.Name, pkg, shipped)
	b.NL()

	b.Comment("The values of " + enum.Name + ".")
	b.L("const (")
	for _, v := range enum.Values {
		b.L("%s%s = %s.%s%s", enum.Name, v.Name, pkg, shipped, v.Name)
	}
	b.L(")")
	b.NL()
