...
func (e *emitter) shutdownBudgetFunc(b *gobuf.Buf) {
	...
	b.Comment("ShutdownBudget is [github.com/simonjanss/rig/runtime/serve.Config.MaxShutdown] " +
		"for this project.\n\n" +
		...
		"terminationGracePeriodSeconds, which is why it is stated here in words " +
		"as well as summed below — an operator reading a manifest should not have " +
		"to run the binary to learn it.\n\n" +
		"A project that registers closers of its own adds them rather than adding " +
		"all of this up again:\n\n" +
		"\tMaxShutdown: api.ShutdownBudget() + 5*time.Second\n\n" +
		"It is not set for you, and serve.Config.MaxShutdown says why: it is a " +
		"stated maximum rather than the sum of its parts, so that it can be read " +
		"off and copied. A field filled in silently is a number nobody can find.\n\n" +
		"Every step this project's configuration registers is counted, including " +
		"one in a process that never starts it — a `Tasks:` entry never reaches " +
		"the mount closure, and serve.Config is built before it either way. It is " +
		"a maximum, so counting a step that is not there costs nothing but " +
		"headroom, and serve.App is the backstop regardless: it adds up what was " +
		"actually registered, before the server listens, and refuses a budget " +
		"that cannot hold it with the parts named.")
