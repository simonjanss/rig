...
//
// It also registers what it found, in the two places it belongs: the
// monitoring page gets the probe itself, so a sync service that comes back
// shows up there without any subscriber having to discover it, and — only
// when this project said it is required — so does the readiness check.
func CheckSyncService(ctx context.Context, app *serve.App, page *observe.Page, proxy *electric.Proxy) error {
	if proxy == nil {
	...
	// The probe rather than a state, so the pill on the monitoring page answers
	// whether the sync service is there now instead of whether it was there when
	// something last happened to touch it. A nil page — a caller using Mount
	// rather than Process.Mount — registers nothing.
	page.Watch("sync service", proxy.Health)
