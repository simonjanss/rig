...
//
// For this project that is 25s: 5s for the presence sweeper, 5s for the live
...
//
//	MaxShutdown: 30 * time.Second // 25s here, plus 5s for a closer of my own
//
...
func ShutdownBudget() time.Duration {
	return presenceShutdown + shapesShutdown + authShutdown + shutdownHeadroom
}
...
//		// ...
//		Shutdown: api.Shutdown{Presence: 2 * time.Second},
//	}, build)
//
...
//
// The fields are Presence, Shapes and Auth, and there are no others because
...
func (s Shutdown) Budget() time.Duration {
	return cmp.Or(s.Presence, presenceShutdown) + cmp.Or(s.Shapes, shapesShutdown) + cmp.Or(s.Auth, authShutdown) + shutdownHeadroom
}
