...
func Register(h Handlers) *http.ServeMux {
	...
	if h.BlogPost != nil {
		registerBlogPost(routes, h.Server, h.BlogPost)
	}
	if h.RigAccount != nil {
	...
	if h.Shapes.Proxy != nil {
		...
		if h.Shapes.RigNotificationRecipientDeleted == nil {
			h.Shapes.RigNotificationRecipientDeleted = RigNotificationRecipientDeletedScope(h.Shapes.RigNotificationRecipient)
		}

		routes.HandleFunc("GET /api/v1/rig_notification_recipient/_stream", handleRigNotificationRecipientShape(h.Server, h.Shapes))
		routes.HandleFunc("GET /api/v1/rig_notification_recipient/_deleted/_stream", handleRigNotificationRecipientDeletedShape(h.Server, h.Shapes))
