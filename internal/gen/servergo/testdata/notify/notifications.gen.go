...
// notifyBlogPostDeletes keeps BlogPost's notifications in step with its rows.
//
...
// Restore.After hook:
//
//	svc.Restoring(ctx, api.NotifyAboutBlogPost(row.ID))
func notifyBlogPostDeletes(svc *notify.Service) dbhook.ChildDelete[model.BlogPostDeleteInput, model.BlogPost] {
	return dbhook.ChildDelete[model.BlogPostDeleteInput, model.BlogPost]{
		Child: "blog_post_notification",
		Deleting: func(ctx context.Context, _ tenancy.Claims, row *model.BlogPost, in model.BlogPostDeleteInput) error {
