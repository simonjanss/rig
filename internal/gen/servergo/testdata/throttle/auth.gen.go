...
func BaseURL() (string, error) {
	return strings.TrimRight(cmp.Or(os.Getenv(BaseURLEnv), "https://app.example.com"), "/"), nil
}
...
type OAuthCredentials struct {
	...
	// GitHub's pair. Empty reads GH_APP_ID and GH_APP_SECRET.
	GitHub OAuthClient
}
...
func ConfiguredProviders(h OAuthHooks) ([]oauth.Provider, error) {
	...
	if id, secret := cmp.Or(h.Credentials.Google.ID, os.Getenv("GOOGLE_CLIENT_ID")), cmp.Or(h.Credentials.Google.Secret, os.Getenv("GOOGLE_CLIENT_SECRET")); id != "" && secret != "" {
		...
		return nil, errors.New("api: provider google is required, but neither Hooks.OAuth.Credentials.Google nor GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET name a complete pair")
	}
	...
	if id, secret := cmp.Or(h.Credentials.Microsoft.ID, os.Getenv("MICROSOFT_CLIENT_ID")), cmp.Or(h.Credentials.Microsoft.Secret, os.Getenv("MICROSOFT_CLIENT_SECRET")); id != "" && secret != "" {
		...
		out = append(out, oauth.Microsoft(id, secret, cmp.Or(h.Credentials.Microsoft.Tenant, os.Getenv("WORK_DIRECTORY"))))
	}

	if id, secret := cmp.Or(h.Credentials.GitHub.ID, os.Getenv("GH_APP_ID")), cmp.Or(h.Credentials.GitHub.Secret, os.Getenv("GH_APP_SECRET")); id != "" && secret != "" {
