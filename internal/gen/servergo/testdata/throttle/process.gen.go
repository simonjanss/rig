...
func Tasks(own map[string]serve.Task) map[string]serve.Task {
	out := map[string]serve.Task{
		...
		// Counters for windows that have closed. Zero takes the default: far enough
		// back that no limit this project states is still counting.
		"sweep-throttle": ThrottleSweeper(0),
	}
...
//
// For this project that is 15s: 5s for the auth cache's invalidation channel
// and 10s left over.
//
